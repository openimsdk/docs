...
The WASM SDK has no general change event for arbitrary public user profiles. `CbEvents.OnSelfInfoUpdated` carries only the signed-in user's `SelfUserInfo`; do not write it into another user's `PublicUserItem` cache. See [Update your profile](/sdk/wasm/user/profile/set-self-info) for reading, updating, and synchronizing the signed-in user's profile.
