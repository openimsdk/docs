...
```ts showLineNumbers
OpenIMSDKRN.removeBlack( userID: string,operationID: string)
```
...
try {
  await OpenIMSDKRN.getFriendList('userID','operationID');
  console.log("getFriendList: Retrieved friend list");
} catch (error) {
