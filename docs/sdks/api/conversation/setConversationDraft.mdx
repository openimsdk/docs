...
```ts showLineNumbers
IMSDK.asyncApi('setConversationDraft', operationID: string, conversationID: string, draftText: string): Promise<void>
```
...
IMSDK.asyncApi('setConversationDraft', Date.now(), 'conversationID', 'draft')
  .then(({ data }) => {
    // 调用成功
  })
  .catch(({ errCode, errMsg }) => {
