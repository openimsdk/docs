...
```ts showLineNumbers
OpenIMSDKRN.getTotalUnreadMsgCount(
  operationID: string
);
...
try {
  ...
  const data = await OpenIMSDKRN.getTotalUnreadMsgCount(operationID);

  console.log("getTotalUnreadMsgCount", data);
} catch (error) {
  console.error('Error getTotalUnreadMsgCount:', error);
}
