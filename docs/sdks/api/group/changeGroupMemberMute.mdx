...
try {
  ...
  await OpenIMSDKRN.changeGroupMemberMute(options, operationID);

  console.log("changeGroupMemberMute: Muted user", options.userID, "in group", options.groupID, "for", options.mutedSeconds, "seconds");
} catch (error) {
  console.error('Error changeGroupMemberMute:', error);
}
