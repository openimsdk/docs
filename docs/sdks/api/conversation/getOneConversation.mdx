...
```ts showLineNumbers
IMSDK.asyncApi('getOneConversation', operationID: string, sourceID: string, sessionType: SessionType): Promise<ConversationItem>
```
...
IMSDK.asyncApi('getOneConversation', Date.now(), 'userID', 1)
  .then(({ data }) => {
    // 调用成功
  })
  .catch(({ errCode, errMsg }) => {
