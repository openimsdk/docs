...
搜索范围取决于 SDK 已同步到本地的消息。服务端全量检索或跨会话全局排序不属于本方法的能力范围；这类结果可以由后端搜索服务返回对应的 `conversationID` 和 `clientMsgID`，再由客户端定位。
...
| `keywordList` | `NSArray<NSString *> *` | 否 | 关键词数组；当前只支持一个关键词。 |
| `keywordListMatchType` | `NSInteger` | 值类型 | 多关键词匹配字段，当前未使用。 |
| `senderUserIDList` | `NSArray * _Nullable` | 是 | 发送者筛选字段，当前未使用。 |
...
`keywordListMatchType` 和 `senderUserIDList` 当前未参与搜索。多关键词或发送者筛选可由其他业务搜索能力提供。

## 处理分页结果

成功 callback 参数可以为 `nil`，应先处理空结果。非空的 `OIMSearchResultInfo` 包含：
...
搜索查询成功后可以直接展示 callback 返回的结果；查询本身不触发消息事件。命中的消息可能在页面打开后被撤回、删除，或因后续同步而变化，应复用[接收消息](/sdk/ios/message/receiving-messages/receive-messages)、[批量删除消息](/sdk/ios/message/managing-messages/delete-saved-messages)和[撤回消息](/sdk/ios/message/managing-messages/revoke-a-message)页面的统一事件处理器，本页不重复注册事件。
...
搜索 callback、后续消息事件和重新执行查询是三条独立路径。需要当前结果时，使用相同条件重新搜索，并按相同的会话与消息标识合并。
