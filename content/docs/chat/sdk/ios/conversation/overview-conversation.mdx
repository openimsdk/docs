...
| `isPrivateChat` / `burnDuration` <span className="enterprise-field-badge">Enterprise</span> | The private-chat state and burn-after-reading duration. |
| `isMsgDestruct` / `msgDestructTime` <span className="enterprise-field-badge">Enterprise</span> | Whether scheduled server-message deletion is enabled and its deletion interval. |
