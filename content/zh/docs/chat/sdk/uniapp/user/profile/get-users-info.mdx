...
| `attachedInfo` <span className="enterprise-field-badge">商业版</span> | `string` 或 `null`（可选） | 仅按已确认的商业业务协议解析。 |
| `globalRecvMsgOpt` <span className="enterprise-field-badge">商业版</span> | `number` 或 `null`（可选） | 账号全局消息接收选项；陌生人资料卡通常不需要展示。 |
