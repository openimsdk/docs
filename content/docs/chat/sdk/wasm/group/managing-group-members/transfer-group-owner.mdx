...
For the complete listener and cleanup implementation, see [List group members](/sdk/wasm/group/retrieving-group-members/get-group-member-list). Do not infer the transfer result from the number of events alone. When you need the current owner and administrators, call `getGroupMemberOwnerAndAdmin(groupID)`. After a new login or reconnection, query the current members and their roles first, then merge later changes from member events into the list.
