...
`open-im-sdk-wasm` is designed for most browser scenarios, while `open-im-sdk` is specifically for mini-programs. The wsAddr parameter during login differs between the two, so be careful not to mix them up.
For `open-im-sdk-wasm`, the wsAddr is usually `ws://your-server-ip:10001`, and the default port used is `10001`.
...
const config = {
    ...
    wsAddr: string;    // IM ws address, generally `ws://your-server-ip:10001`(open-im-sdk-wasm) or `ws://your-server-ip:10003`(open-im-sdk)
}
IMSDK.login(config)
  .then(() => {
    // Login successful
...
| Parameter Name    | Parameter Type | Mandatory | Description                                                               |
| ----------- | -------- | -------- | ------------------------------------------------------------------- | --- |
| userID      | String   | Yes       | IM 用户 userID                                                      |
| token       | String   | Yes       | OpenIM 用户令牌，业务后台验证用户账号密码后，通过 user_token 来获取 |     |
