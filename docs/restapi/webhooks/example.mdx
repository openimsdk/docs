...
使用 OpenIM 中的 callback 机制实现聊天机器人问答功能，即给聊天机器人发送一条消息后（支持发送文本消息和图片消息），聊天机器人把这条消息返回。
...
tips:
>1、需要将例子中的 **robotics** 替换成步骤二中获取的 userID
...
```
[详细代码请参考](https://github.com/luhaoling/chat/blob/logs/internal/api/callbackExample.go)

## 4.体验该功能
1、登录成功可以给聊天机器人账号发送消息和图片
