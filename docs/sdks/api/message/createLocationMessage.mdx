...
```ts showLineNumbers
IMSDK.asyncApi('createLocationMessage', operationID: string, description: string,latitude: number,
  longitude: number): Promise<MessageItem>
```
...
IMSDK.asyncApi('createLocationMessage', Date.now(), 'description', 0, 0)
  .then(({ data }) => {
    // 调用成功
  })
  .catch(({ errCode, errMsg }) => {
