...
Use this policy to display the appropriate application prompt, but OpenIMServer makes the final admission decision. After the Promise succeeds, merge the latest group profile from `OnGroupInfoChanged`; see [Group overview](/sdk/wasm/group/overview-group) for the complete listener.
