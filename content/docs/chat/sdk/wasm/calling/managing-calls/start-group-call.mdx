...
Generate a new `roomID` for every group call instead of reusing `groupID`; otherwise, separate calls in the same group can be merged incorrectly. Invitees obtain this value from the invitation event, and subsequent accept, reject, cancel, and hang-up operations must keep using the same `roomID`. Remove the current user, empty values, and duplicates from the invitee list before starting the call.
