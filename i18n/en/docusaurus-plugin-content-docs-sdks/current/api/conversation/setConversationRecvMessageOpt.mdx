...
| status             | [int](/enum/recvMsgOpt.mdx)           | 是       | 消息免打扰状态 0：正常；1：不接受消息；2：接受在线消息不接受离线消息 |
...
| status          | [OIMReceiveMessageOpt](/enum/recvMsgOpt.mdx) | 是       | 消息免打扰状态 |
...
| 参数名称        | 参数类型                                         | 是否必填 | 描述                                                               |
| --------------- | ------------------------------------------------ | -------- | ------------------------------------------------------------------ |
| base            | [OnBase<String\>](/callback/onBase.mdx) | 是       | 回调接口                                                           |
| conversationIDs | List<String\>                                    | 是       | 会话 ID 列表                                                       |
| status          | int                                              | 是       | [Opt](/enum/recvMsgOpt.mdx) 接收消息状态 |
...
| -------------- | ------------------------------------------------------ | -------- | -------------- |
| conversationID | string                                                 | 是       | 会话 ID        |
| opt            | [MessageReceiveOptType](/enum/recvMsgOpt.mdx) | 是       | 消息免打扰状态 |
