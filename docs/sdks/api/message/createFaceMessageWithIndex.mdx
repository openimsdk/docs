...
```ts showLineNumbers
IMSDK.asyncApi('createFaceMessage', operationID: string, index: number,
  dataStr: string): Promise<MessageItem>
```
...
IMSDK.asyncApi('createFaceMessage', Date.now(), 0, 'https://xxx/xxx/xx.png')
  .then(({ data }) => {
    // 调用成功
  })
  .catch(({ errCode, errMsg }) => {
