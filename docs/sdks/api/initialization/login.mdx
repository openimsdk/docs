...
`@openim/wasm-client-sdk`是针对大部分浏览器场景下的使用方案，而`open-im-sdk`只是针对小程序使用，两者之间登录时参数中的`wsAddr`不同，注意不要混淆。   
`@openim/wasm-client-sdk`对应的`wsAddr`一般为`ws://your-server-ip:10001`,使用的端口默认为`10001`。   
...
const config = {
    ...
    wsAddr: string;    // IM ws 地址，一般为`ws://your-server-ip:10001`(@openim/wasm-client-sdk)或`ws://your-server-ip:10003`(open-im-sdk)
}
IMSDK.login(config)
  .then(() => {
    // 登录成功
...
| token       | string   | 是       | OpenIM 用户令牌，业务后台验证用户账号密码后，通过 user_token 来获取 |     |
| cb          | [OnSucOrError](docs/sdks/callback/OnSucOrError.mdx)| 是       | 登录成功或失败回调 |     |
