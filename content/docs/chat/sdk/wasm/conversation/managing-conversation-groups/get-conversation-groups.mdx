...
This query establishes a snapshot only and does not trigger a group event. Merge later changes from incremental events and query again when needed for reconciliation. See [Conversation group overview](/sdk/wasm/conversation/managing-conversation-groups/overview-conversation-groups) for the event list.
