...
- **REST API:** Lets a trusted backend call OpenIMServer with administrator privileges to manage and extend business functionality.
- **Webhooks:** When specified events occur before or after an operation, OpenIMServer proactively sends HTTP or HTTPS requests to your business server. Your server can use these callbacks to intervene in the subsequent flow or synchronize data.
...
- **Self-hosted private deployment:** Run OpenIMServer and, when required, ChatServer on premises, in a private cloud, or in a localized technology environment. Docker Compose, source deployment, and cluster deployment are supported.
