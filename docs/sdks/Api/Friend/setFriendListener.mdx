...
JS SDK 不需要主动设置，默认已经启用，可以直接在客户端使用监听，[用户事件详情参见](docs/sdks/Listener/FriendshipListener.mdx)
...
设置成功后回调会通过`globalEvent`传递到客户端，[用户事件详情参见](docs/sdks/Listener/FriendshipListener.mdx)
...
IMSDK.nomalApi.setFriendListener(): void;
...
IMSDK.nomalApi.setFriendListener();
```
