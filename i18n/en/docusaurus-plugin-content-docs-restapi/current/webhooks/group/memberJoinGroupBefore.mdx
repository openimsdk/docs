...
The example below shows the callback URL configured for the app as `http://{WEBHOOK_ADDRESS}/callbackCommand?contenttype=json`.
...
| {WEBHOOK_ADDRESS}    | Domain or hostname in the `callback.url` field of `configy.yaml` |
