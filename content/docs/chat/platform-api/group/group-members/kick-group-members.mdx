...
description: "Kick group members with the OpenIM Platform API, including endpoint, request body, response fields, and error handling."
...
version: "v3"
sourcePath: "/platform-api/group/group-members/kick-group-members"
---

Use **Kick group members** to remove one or more users from a specified group.

## HTTP request

```bash
POST {API_ADDRESS}/group/kick_group
```

### Request example

```bash
curl --request POST "${API_ADDRESS}/group/kick_group" \
...
| kickedUserIDs | Yes | array | Users to remove, up to 1,000. |
| deleteMember <span className="enterprise-field-badge">Enterprise</span> | No | boolean | Whether to also delete the removed users' group-member data. |
