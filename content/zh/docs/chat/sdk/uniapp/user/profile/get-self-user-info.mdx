...
| `globalRecvMsgOpt` <span className="enterprise-field-badge">商业版</span> | `number` 或 `null`（可选） | 当前账号的全局消息接收方式；常量和语义见[设置全局消息接收方式](/sdk/uniapp/user/profile/set-global-message-reception)。 |
| `attachedInfo` <span className="enterprise-field-badge">商业版</span> | `string` 或 `null`（可选） | SDK 附加信息；只按已确认的业务约定解析。 |
