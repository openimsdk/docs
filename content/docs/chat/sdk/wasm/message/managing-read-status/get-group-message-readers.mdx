...
When the Promise resolves, `data` is the current page of `GroupMemberItem[]` that matched when the query ran. For field descriptions, see [List group members](/sdk/wasm/group/retrieving-group-members/get-group-member-list). Deduplicate by `groupID:userID`. The query does not trigger a receipt event. Continue paging when the complete list is required.
