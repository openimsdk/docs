...
- 如果您需要基于 OpenIMSDK 开发新特性，首先要确定是针对业务侧还是即时通讯核心逻辑。
- 由于 OpenIMSDK 系统本身已经做好了比较多的抽象，大部分聊天的功能已经具备了，不建议修改直接修改OpenIMSDK。
...
我们需要对 IMServer 下发的通知进行处理，需要在 `internal/relation/notification.go` 中实现对应的通知处理方法。
