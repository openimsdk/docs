...
### 简要描述
- 以普通用户、系统通知号或APP管理员身份向指定用户或群组发送消息，也可用于从其他平台导入历史记录。
### 请求方式
- `post` 
### 请求URL
...
|字段名|选填|类型|说明|
|:----    |:-------    |:--- |---|
|sendID|必填|string|发送者ID(APP管理员ID、用户ID或系统通知号)|
...
|senderPlatformID|选填|int|发送者平台号，模拟用户发送时填写， 1：IOS，2：Android，3：Windows，4：OSX，5：Web，6：MiniWeb，7：Linux，8：Android Pad，9：IPad，10：admin|
...
|sessionType|必填|int|会话类型,1：单聊，2：群聊，4：系统通知|
