...
1.编辑basic-openim-server-dependency.yml 在此文件中注释掉mysql相关配置项
...
```bash
不要直接修改config.yaml文件，建议编辑scripts/install/environment.sh后 make init 使配置生效
 ```
...
OpenIM 是开源的即时通讯组件，包括open-im-sdk和open-im-server ，它并不是一个独立的产品，本身不包含账号的注册和登录服务。
...
```bash
	确认zookeeper的ip端口是否正确
```

2. 如果注册失败

```bash
	确认openIMUrl的ip端口是否正确
```
