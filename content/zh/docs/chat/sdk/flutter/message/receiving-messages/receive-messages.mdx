...
根据 `contentType` 或非空内容元素选择渲染器：`textElem`、`atTextElem`、`customElem` 分别处理文本、@ 和自定义内容；`pictureElem`、`soundElem`、`videoElem`、`fileElem` 读取消息中已有的 URL、大小、名称、时长或封面图，不需要在接收端重新上传。未知类型应显示安全的“不支持消息”占位，不能把任意自定义数据直接当 HTML 渲染。
...
当前 SDK 中用于接收、路由与合并的常用字段如下，除 `exMap` 外都声明为 nullable，因此 listener 不能假设回调数据完整：
...
这三个函数应纳入应用唯一的 `OnAdvancedMsgListener`，集中设置方式见[事件概览](/sdk/flutter/events/overview-events)。固定 SDK 没有 remove 或 unset API。
...
- 验证缺少必要路由字段或 `clientMsgID` 的异常消息被忽略并记录诊断，而不是造成崩溃。
