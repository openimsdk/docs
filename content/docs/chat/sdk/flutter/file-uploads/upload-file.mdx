...
The Future, progress callbacks, and completion callback are separate stages. Use the URL from the completion callback to update an avatar or another business resource. This method does not automatically update a profile, create a message, or send a message. See [Listen for upload progress and completion](/sdk/flutter/file-uploads/set-upload-file-listener) for the single listener setup.
