...
```ts showLineNumbers
IMSDK.asyncApi('pinConversation', operationID: string, conversationID: string, isPinned: boolean): Promise<void>
```
...
IMSDK.asyncApi('pinConversation', Date.now(), 'conversationID', true)
  .then(({ data }) => {
    // 调用成功
  })
  .catch(({ errCode, errMsg }) => {
