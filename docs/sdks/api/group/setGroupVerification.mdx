...
| needVerification | [int](docs/sdks/enum/groupVerification.mdx)      | 是       | 入群验证方式 |
...
| needVerification | [OIMGroupVerificationType](docs/sdks/enum/groupVerification.mdx) | 是       | 入群验证方式 |
...
```ts showLineNumbers
IMSDK.asyncApi('setGroupVerification', operationID: string, {
  groupID: string;
  verification: GroupVerificationType;
}): Promise<void>
```
...
IMSDK.asyncApi('setGroupVerification', IMSDK.uuid(), {
  groupID: '',
  verification: GroupVerificationType.AllNeed,
})
  .then(() => {
