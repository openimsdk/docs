...
try {
  const options = {
    ...
    isMute: true,               // Replace with the actual mute status (true or false)
  };
  const operationID = 'OperationID_value';

  await OpenIMSDKRN.changeGroupMute(options, operationID);

  const muteStatus = options.isMute ? 'muted' : 'unmuted';
  console.log("changeGroupMute: Group", options.groupID, "is", muteStatus);
} catch (error) {
  console.error('Error changeGroupMute:', error);
}
