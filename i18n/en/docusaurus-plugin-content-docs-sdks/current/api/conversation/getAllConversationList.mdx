...
```ts showLineNumbers
OpenIMSDKRN.getAllConversationList(
  operationID: string
);
...
| Promise.catch() | Promise<[Response](docs/sdks/class/response.mdx)\>                                          | Callback on failed call |
...
try {
  ...
  const data = await OpenIMSDKRN.getAllConversationList(operationID);
