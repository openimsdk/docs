...
```ts showLineNumbers
OpenIMSDKRN.getFriendApplicationListAsRecipient(
  operationID: string
);
...
| Parameter Name        | Parameter Type                                                                          | Description              |
| --------------- | --------------------------------------------------------------------------------- | ------------------ |
| Promise.then()  | Promise<[FriendApplicationItem](docs/sdks/class/friend/friendApplication.mdx)[]\> | 收到的好友请求列表 |
...
try {
  ...
  const data = await OpenIMSDKRN.getFriendApplicationListAsRecipient(operationID);

  console.log("getFriendApplicationListAsRecipient: Retrieved friend applications as the recipient", data);
} catch (error) {
