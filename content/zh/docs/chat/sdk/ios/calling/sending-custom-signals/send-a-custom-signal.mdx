...
自定义信令只提供实时通知，不是可查询的持久业务状态。页面重新进入、设备离线或 listener 尚未注册时，都可能无法仅凭它恢复完整状态。需要长期保存或审计的数据应通过业务后端或普通消息能力承载；通话房间和参与者快照使用[按群组查询房间](/sdk/ios/calling/retrieving-call-information/get-room-by-group-id)校准。
