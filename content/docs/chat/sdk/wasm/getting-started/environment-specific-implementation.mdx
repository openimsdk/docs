...
Commit the dependency lockfile for team projects. When the application must pin the SDK, select a version that your team has verified instead of copying a long-lived version number from this page.
...
- Network changes, page foreground/background transitions, and focus recovery should be handled centrally by the SDK wrapper. Business components should not register duplicate low-level runtime listeners.
...
| No successful connection after `login()` | `apiAddr`, `wsAddr`, the token, the platform ID, or network policy is incorrect. | Register connection events first, then use `errCode`, `errMsg`, and server logs to locate the cause. |
