...
A successful callback means that the announcement update request completed; it does not mean that every client has updated its UI. `onGroupInfoChanged` provides the updated `GroupInfo`. Merge the announcement and group profile by `groupID`. See [Group overview](/sdk/android/group/overview-group) for the complete listener setup.
