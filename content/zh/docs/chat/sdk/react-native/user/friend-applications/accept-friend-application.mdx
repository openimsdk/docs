---
title: '同意好友申请'
description: 'OpenIM React Native SDK 同意好友申请。'
product: 'sdk'
context: 'chat/sdk/react-native'
template: 'guide'
status: 'draft'
version: 'v4'
platform: 'react-native'
sourcePath: '/sdk/react-native/user/friend-applications/accept-friend-application'
---

调用 `acceptFriendApplication()` 同意好友申请。

```ts
await OpenIMSDK.acceptFriendApplication({
  toUserID: applicantUserID,
  handleMsg: '',
});
```

## 参数说明

| 参数 | 类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| `toUserID` | `string` | 是 | 申请发起人的用户 ID。 |
| `handleMsg` | `string` | 是 | 处理申请时附带的说明，没有内容时传空字符串。 |

同意后会建立好友关系，并触发好友申请状态变化和 `OnFriendAdded` 事件。好友列表变化的监听方式见[获取好友列表](/sdk/react-native/user/friends/get-friend-list-page)。
