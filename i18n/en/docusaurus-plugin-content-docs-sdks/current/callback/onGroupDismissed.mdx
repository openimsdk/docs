---
sidebar_position: 3
toc_min_heading_level: 2
toc_max_heading_level: 2
---

# onGroupDismissed

## 功能介绍

:::info

群组被解散时，该群所有群成员会收到此回调。

:::

<Tabs
groupId="sdks-language"
values={[
{ label: 'iOS', value: 'iOS', },
{ label: 'Android', value: 'Android', },
{ label: 'Flutter', value: 'Flutter', },
{ label: 'uni-app', value: 'uni-app', },
{ label: 'Web', value: 'Web', },
]
}>

<TabItem value="Flutter">

### 返回原型

```dar showLineNumbers
  Function(GroupInfo info)? onGroupDismissed;
```

### 返回结果

| 名称      | 类型                                             | 描述     |
| --------- | ------------------------------------------------ | -------- |
| groupInfo | [GroupInfo](/class/group/groupInfo.mdx) | 群聊信息 |

</TabItem>

<TabItem value="iOS">

### 返回原型

```swift showLineNumbers

- (void)onGroupDismissed:(OIMGroupInfo *)changeInfo;

```

### 返回结果

| 名称      | 类型                                                | 描述     |
| --------- | --------------------------------------------------- | -------- |
| groupInfo | [OIMGroupInfo](/class/group/groupInfo.mdx) | 群聊信息 |

</TabItem>

<TabItem value="Android">

### 返回原型

```java showLineNumbers
   void onGroupDismissed(GroupInfo info);
```

### 返回结果

| 名称      | 类型                                             | 描述   |
| --------- | ------------------------------------------------ | ------ |
| info | [GroupInfo](/class/group/groupInfo.mdx) | 组信息 |


</TabItem>

<TabItem value="Web">

### 返回原型

```ts showLineNumbers

onGroupDismissed(data: WSEvent<GroupItem>): void;

```

### 返回结果

| 名称 | 类型                                                                                      | 描述     |
| ---- | ----------------------------------------------------------------------------------------- | -------- |
| data | [WSEvent](/class/response.mdx)<[GroupItem](/class/group/groupInfo.mdx)> | 群聊信息 |

### 调用示例

```js showLineNumbers
import { getSDK } from 'open-im-sdk-wasm';
import { CbEvents } from 'open-im-sdk-wasm/constant';
const IMSDK = getSDK();

IMSDK.on(CbEvents.OnGroupDismissed, ({ data }) => {
  // data 群信息
});
```

</TabItem>

<TabItem value="uni-app">

### 返回原型

```ts showLineNumbers

onGroupDismissed(data: WSEvent<GroupItem>): void;

```

### 返回结果

| 名称 | 类型                                                                                      | 描述     |
| ---- | ----------------------------------------------------------------------------------------- | -------- |
| data | [WSEvent](/class/response.mdx)<[GroupItem](/class/group/groupInfo.mdx)> | 群聊信息 |

### 调用示例

```js showLineNumbers
import IMSDK from 'openim-uniapp-polyfill';

IMSDK.subscribe(IMSDK.IMEvents.OnGroupDismissed, ({ data }) => {
  // data 群信息
});
```

</TabItem>

</Tabs>
