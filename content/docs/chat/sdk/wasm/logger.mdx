...
WASM SDK logs help diagnose browser sign-in and business API calls. Use the log level and standard-output options to control the amount of SDK output.
...
| Reduced output | Use `Warn` or `Error` and disable console output | Emit only SDK logs at the selected levels. |
...
| `isLogStandardOutput` | `boolean` | No | Whether to write SDK logs to the browser console. |
