...
## Description
The App backend can use this callback to take action on Group Chat messages in real-time, such as:
- Logging Group Chat messages.
...
## Timing of Callback
- After OpenIMServer receives the user’s Group Chat message but before distributing it to the target user.
...
| callbackCommand     | string   | Callback command, used for callback before sending a Group Chat message |
