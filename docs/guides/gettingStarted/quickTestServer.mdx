...
| TCP 端口  | 说明                                                  | 操作 ⚙️                                      |
|:--------:|:-----------------------------------------------------:|:--------------------------------------------:|
| TCP:10001| ws 协议，消息端口，用于客户端 SDK                     | 端口放行或 nginx 反向代理，并关闭防火墙     |
...
:::tip
在浏览器中输入 `http://ip:11001` 来访问 PC Web。此 ip 为服务端 ip，且确保浏览器能访问。首次使用请通过手机号注册，验证码默认为 `666666`。
:::

import Image3 from './assets/pc-web.png';

<img src={Image3} width="400" alt="PC Web Interface" />
