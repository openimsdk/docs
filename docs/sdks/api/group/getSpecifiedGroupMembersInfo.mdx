...
```ts showLineNumbers
IMSDK.asyncApi('getSpecifiedGroupMembersInfo', operationID: string, groupID: string, userIDList: string[]): Promise<GroupMemberItem[]>
```
...
IMSDK.asyncApi('getSpecifiedGroupMembersInfo', Date.now(), 'groupID', [
  'userID',
])
  .then((data) => {
    // 调用成功
