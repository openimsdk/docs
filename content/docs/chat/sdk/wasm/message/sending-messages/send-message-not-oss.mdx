...
When the Promise resolves, `data` is the server-confirmed `MessageItem`. Use `clientMsgID` to find and update the local pending message. For field descriptions, see [Message overview](/sdk/wasm/message/overview-message).
