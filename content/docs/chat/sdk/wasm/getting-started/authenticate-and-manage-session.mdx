...
3. Obtain the `userID`, token, `apiAddr`, and `wsAddr` described in “Before you start” from the application backend.
...
| `debug` | `boolean` | No | Controls diagnostic output from the JavaScript wrapper and local database bridge. It is enabled by default. |
...
Register connection events before calling `login()`. This lets the application capture errors caused by network access, service addresses, tokens, or server conditions during sign-in and update its connection state.
...
| `token` | `string` | Yes | The current user's OpenIMSDK token returned by the application backend. It must correspond to `userID`. |
...
| `logLevel` | `LogLevel` | No | The SDK log level. See [Logging](/sdk/wasm/logger) for enum values. |
...
`getLoginUserID()` returns the user ID currently signed in to the SDK. Use it to verify that the application account and SDK account match; it does not replace application authentication. When either query Promise succeeds, use its `data` value to restore the current sign-in state. The queries themselves do not trigger connection events.
...
The application backend issues the OpenIMSDK token and returns it to the browser. The frontend passes it to `login()` and restarts authentication when the token expires, becomes invalid, or the user switches accounts.
...
The `data` value of `OnUserTokenInvalid` is a reason string returned by the SDK. To refresh a token, request a new `userID`, token, and service addresses from the application backend, then call `login()` again through the application's authentication flow.
...
When `OnKickedOffline` arrives, the WASM SDK has already signed out the current session automatically. Do not call `logout()` again. Clear the current user, conversation list, message view, and page state maintained by the application, then enter its signed-out flow.
...
This page provides the complete listener examples for connection, token, and account sign-out events. When signing out, switching accounts, or destroying the SDK scope, remove them with the same function references used during registration:
...
Connection events do not correspond to an individual user, conversation, or message record. Isolate their state by SDK instance and signed-in user, and remove the old instance's listeners before switching accounts. After signing in again, each feature uses its own events to synchronize changes. Query and display the data required by a page when it first opens.
