...
```ts showLineNumbers
IMSDK.asyncApi('getConversationListSplit', operationID: string, offset: number, count: number): Promise<ConversationItem[]>
```
...
IMSDK.asyncApi('getConversationListSplit', Date.now(), 0, 20)
  .then(({ data }) => {
    // 调用成功
  })
  .catch(({ errCode, errMsg }) => {
