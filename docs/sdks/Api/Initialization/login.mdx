...
const config = {
    ...
    apiAddress: string;   // IM api 地址，一般为`http://xxx:10002`或`https://xxx/api
    wsAddress: string;    // IM ws 地址，一般为`ws://xxx:10001`或`wss://xxx/message_gateway`
    logLevel?: number;    // SDK 日志打印级别
}
IMSDK.login(config)
  .then(() => {
    // 登录成功
...
```ts showLineNumbers
IMSDK.asyncApi('login', operationID: string,userID: string, token: string): Promise<void>
```
...
IMSDK.asyncApi('login', Date.now(), userID, token)
  .then(() => {
    // 登录成功
  })
  .catch(({ errCode, errMsg }) => {
