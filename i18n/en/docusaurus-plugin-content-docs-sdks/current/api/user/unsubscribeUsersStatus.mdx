...
```ts showLineNumbers
OpenIMSDKRN.unsubscribeUsersStatus(userIDList: string[],operationID: string)
```
...
try {
  ...
  const data = await OpenIMSDKRN.unsubscribeUsersStatus(
    userIDList,
    operationID
  );

  console.log("unsubscribeUsersStatus: ",data);
} catch (error) {
  console.error('Error unsubscribeUsersStatus:', error);
}
