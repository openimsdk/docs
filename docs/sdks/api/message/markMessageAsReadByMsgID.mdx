...
| clientMsgIDs   | NSArray <NSString \*> | 是       | clientMsgID 的列表 |
...
[OIMManager.manager markMessageAsReadByMsgID:@""
                                   msgIDList:@[]
...
```dart showLineNumbers
    OpenIMClient.getInstance().messageManager.markMessageAsReadByConID(new OnBase<AdvancedMessage>() {
            @Override
...
```ts showLineNumbers
IMSDK.asyncApi('markMessagesAsReadByMsgID', operationID: string, conversationID: string,
  clientMsgIDList: string[]): Promise<void>
```
...
IMSDK.asyncApi('markMessagesAsReadByMsgID', Date.now(), 'conversationID', [
  'clientMsgID',
])
  .then(({ data }) => {
    // 调用成功
