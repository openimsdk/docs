...
- **Definition:** When a user is online, OpenIMServer first delivers messages over the long-lived connection maintained by OpenIMClientSDK. This is called online push.
- **Characteristics:**
...
- **Definition:** When the user is offline or the app process has been terminated, the OpenIMClientSDK long-lived connection cannot deliver the message, so a third-party push service is required.
- **Implementation:**
...
Except in the JavaScript SDK, OpenIMClientSDK fetches and loads messages as needed. Messages already fetched by a client are stored locally and are collectively called local messages.
...
Files, images, and videos are generally uploaded to object storage to obtain a URL before the message is sent. OpenIMClientSDK handles this flow internally, but you must configure one supported object storage service, such as MinIO, OSS, COS, or S3.
