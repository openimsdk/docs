...
以下示例中 App 配置的回调 URL 为 `http://{WEBHOOK_ADDRESS}/callbackCommand?contenttype=json`。

```plaintext
http://{WEBHOOK_ADDRESS}/callbackBeforeUpdateUserInfoExCommand?contenttype=json
```
