...
| `debug` | `boolean` | 否 | 控制 JavaScript 包装层和本地数据库桥接的调试输出，默认开启。 |
...
| `token` | `string` | 是 | 业务后端返回的当前用户 OpenIMSDK Token，必须与 `userID` 对应。 |
...
| `logLevel` | `LogLevel` | 否 | SDK 日志级别，枚举值见[日志](/sdk/wasm/logger)。 |
