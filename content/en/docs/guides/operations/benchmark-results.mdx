## OpenIMSDK stress and reliability testing

## Terminology & Scope

- **OpenIMSDK**: General term, including **OpenIMClientSDK** (client SDK) and **OpenIMServer** (server).
- **ChatServer**: The business extension server. It is separate from the OpenIMServer core benchmark unless explicitly stated.
...
OpenIMSDK is not a standalone chat application such as WeChat or Slack. It is an instant messaging solution that includes OpenIMClientSDK and OpenIMServer. Because OpenIMClientSDK relies on the Go-based `openim-sdk-core` and large fleets of real devices are impractical for repeatable tests, the benchmark programs simulate large-scale online presence and message traffic to evaluate system capacity, reliability, and latency.
...
| openim-push | 13%*8 |
| Other OpenIMServer services and components | 65% |
| Total | 902% |
...
| Other OpenIMServer services and components | 457 MiB |
| Total (all OpenIMServer services and components) | 6.986 GiB |
...
OpenIMServer supports 50,000 concurrent online users and multiple 50,000-member large groups under the tested configuration. At 1,700 messages per second, the measured delivery rate was 100%, average latency remained below one second, and maximum latency remained below three seconds.
