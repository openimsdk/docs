...
`open-im-sdk`和`@openim/wasm-client-sdk` 不需要初始化，直接调用 login 即可。只有在electron中引入`@openim/electron-client-sdk`采用了ffi的方式，才需要调用`initSDK`初始化。
...
|  cb      | [IConnCallBack](docs/sdks/listener/connectListener.mdx)   |   是     | 网络连接状态回调   |
