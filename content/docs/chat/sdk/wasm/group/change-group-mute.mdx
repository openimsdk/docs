...
A successful Promise means that the server completed the group mute update. The group profile may subsequently arrive through `OnGroupInfoChanged`; merge it by `groupID`. See [Group overview](/sdk/wasm/group/overview-group) for the complete listener. Query the group profile again when immediate reconciliation is required.
