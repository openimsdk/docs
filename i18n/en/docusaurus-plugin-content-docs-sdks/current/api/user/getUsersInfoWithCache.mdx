---
sidebar_position: 4
toc_min_heading_level: 2
toc_max_heading_level: 2
---

# getUsersInfoWithCache

## 功能介绍

:::info 说明

获取在IM中的昵称和头像

:::

:::caution 注意

（1）建议一次最大 100 个，因为数量过多可能会导致数据包太大被后台拒绝；   
（2）优先从服务端获取，如果失败，则从本地缓存中获取；   
（3）用户调用此接口获取非好友的个人信息，SDK 内部会更新此缓存。

:::

<Tabs
groupId="sdks-language"
values={[
{ label: 'iOS', value: 'iOS', },
{ label: 'Android', value: 'Android', },
{ label: 'Flutter', value: 'Flutter', },
{ label: 'uni-app', value: 'uni-app', },
{ label: 'Web', value: 'Web', },
]
}>

<TabItem value="Flutter">

### 函数原型

```dart showLineNumbers
Future<List<UserInfo>> getUsersInfoWithCache(
  List<String> userIDs, {
    String? groupID,
    String? operationID,
})
```

### 输入参数

| 参数名称 | 参数类型      | 是否必填 | 描述    |
| -------- | ------------- | -------- | ------- |
| userIDs  | List<String\> | 是       | 用户 ID 列表 |
| groupID  | String        | 否       | 群组 ID |

### 返回结果

| 参数名称 | 参数类型        | 描述         |
| -------- | --------------- | ------------ |
| then     | List<[UserInfo](/class/user/userInfo.mdx) \> | 调用成功回调 |
| onError  | Function        | 调用失败回调 |

### 代码示例

```dart showLineNumbers
await OpenIM.iMManager.userManager.getUsersInfoWithCache([], '');
// todo
```

</TabItem>

<TabItem value="iOS">

### 函数原型

```swift showLineNumbers

- (void)getUsersInfoWithCache:(NSArray<NSString *> *)userIDs
                     groupID:(NSString * _Nullable)groupID
                   onSuccess:(nullable OIMUsersInfoCallback)onSuccess
                   onFailure:(nullable OIMFailureCallback)onFailure;

```

### 输入参数

| 参数名称 | 参数类型              | 是否必填 | 描述    |
| -------- | --------------------- | -------- | ------- |
| userIDs  | NSArray<NSString \*\> | 是       | 用户 ID 列表 |
| groupID  | NSString              | 否       | 群组 ID |

### 返回结果

| 参数名称  | 参数类型                                             | 描述     |
| --------- | ---------------------------------------------------- | -------- |
| onSuccess | [OIMUserInfo](/class/user/userInfo.mdx)                                  | 成功返回 |
| onFailure | OIMFailureCallback | 失败返回 |

### 代码示例

```swift showLineNumbers

[OIMManager.manager getUsersInfoWithCache:@[]
                                 groupID:@""
                               onSuccess:^(NSArray<OIMUserInfo *> * _Nullable usersInfo) {

} onFailure:^(NSInteger code, NSString * _Nullable msg) {

}];

```

</TabItem>

<TabItem value="Android">

### 函数原型

```java showLineNumbers

public void getUsersInfoWithCache(OnBase<List<UserInfo>> callBack, List<String> uidList,String groupID)

```

### 输入参数

| 参数名称 | 参数类型                                                                                     | 是否必填 | 描述         |
| -------- | -------------------------------------------------------------------------------------------- | -------- | ------------ |
| callBack | [OnBase](/callback/onBase.mdx)<List<[UserInfo](/class/user/userInfo.mdx)>> | 是       | 回调接口     |
| uidList  | List<String\>                                                                                | 是       | 用户 ID 集合 |
| groupID  | String                                                                        | false       | 群ID  |
### 返回结果

### 代码示例

```java showLineNumbers
        OpenIMClient.getInstance().userInfoManager.getUsersInfoWithCache(new OnBase<List<UserInfo>>() {
            @Override
            public void onError(int code, String error) {

            }

            @Override
            public void onSuccess(List<UserInfo> data) {

            }
        },uidList,groupId);

```

</TabItem>

<TabItem value="Web">

### 函数原型

```ts showLineNumbers
IMSDK.getUsersInfoWithCache({
  userIDList: string[];
  groupID?: string;
}, operationID?: string): Promise<WsResponse<FullUserItem[]>>
```

### 输入参数

| 参数名称   | 参数类型 | 是否必填 | 描述             |
| ---------- | -------- | -------- | ---------------- |
| userIDList | string[] | 是       | 用户 userID 列表 |
| groupID    | string   | 否       | 群ID            |

### 返回结果

| 参数名称        | 参数类型                                                                      | 描述                 |
| --------------- | ----------------------------------------------------------------------------- | -------------------- |
| Promise.then()  | Promise<WsResponse<[FullUserItem](/class/user/fullUserInfo.mdx)[]>\> | 查询到的用户信息列表 |
| Promise.catch() | Promise<[WsResponse](/class/response.mdx)\>                          | 调用失败回调         |

### 代码示例

```js showLineNumbers
import { getSDK } from 'open-im-sdk-wasm';
const IMSDK = getSDK();

const userIDList = ['userID1', 'userID2'];
IMSDK.getUsersInfoWithCache({
  userIDList,
  groupID: 'groupID',
})
  .then(({ data }) => {
    // data: 查询到的用户信息列表
  })
  .catch(({ errCode, errMsg }) => {
    // 调用失败
  });
```

</TabItem>

<TabItem value="uni-app">

### 函数原型

```ts showLineNumbers
IMSDK.asyncApi('getUsersInfoWithCache', operationID: string, {
  userIDList: string[];
  groupID?: string;
}): Promise<FullUserInfo[]>
```

### 输入参数

| 参数名称    | 参数类型 | 是否必填 | 描述                                                    |
| ----------- | -------- | -------- | ------------------------------------------------------- |
| operationID | string   | 是       | 操作 ID，用于定位问题，保持唯一，建议用当前时间和随机数 |
| userIDList  | string[] | 是       | 用户 userID 列表                                        |
| groupID     | string   | 否       | 群ID                                                   |

### 返回结果

> 通过`openim-uniapp-polyfill`包将函数 Promise 化，调用时需要使用`then`和`catch`判断并处理成功和失败回调。

| 参数名称        | 参数类型                                                          | 描述                 |
| --------------- | ----------------------------------------------------------------- | -------------------- |
| Promise.then()  | Promise<[FullUserInfo](/class/user/fullUserInfo.mdx)[]\> | 查询到的用户信息列表 |
| Promise.catch() | Promise<[CatchResponse](/class/response.mdx)\>           | 调用失败回调         |

### 代码示例

```js showLineNumbers
import IMSDK from 'openim-uniapp-polyfill';

const userIDList = ['userID1', 'userID2'];
IMSDK.asyncApi('getUsersInfo', IMSDK.uuid(), {
  userIDList,
  groupID: 'groupID',
})
  .then((data) => {
    // data: 查询到的用户信息列表
  })
  .catch(({ errCode, errMsg }) => {
    // 调用失败
  });
```

</TabItem>

</Tabs>
