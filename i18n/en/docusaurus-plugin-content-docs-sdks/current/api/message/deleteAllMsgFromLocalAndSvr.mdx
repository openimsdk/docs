...
```ts showLineNumbers
OpenIMSDKRN.deleteAllMsgFromLocalAndSvr(operationID: string);
```
...
try {
  const data = await OpenIMSDKRN.deleteAllMsgFromLocalAndSvr(
    'OperationID_value'
  );
  console.log("deleteAllMsgFromLocalAndSvr", data);
} catch (error) {
  console.error('Error deleteAllMsgFromLocalAndSvr:', error);
}
