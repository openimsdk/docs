...
The returned `Message` is only a local pending object. Sending and receiving clients should process the custom data with the agreed protocol version and field definitions and provide a fallback for unknown versions. Continue with [Send a message](/sdk/android/message/sending-messages/send-message).
