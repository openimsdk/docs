...
description: "Transfer group owner with the OpenIM Platform API, including endpoint, request body, response fields, and error handling."
...
version: "v3"
sourcePath: "/platform-api/group/managing-groups/transfer-group-owner"
---

Use **Transfer group owner** to assign ownership to another existing group member. The former owner becomes a regular member.

## HTTP request

```bash
POST {API_ADDRESS}/group/transfer_group
```

### Request example

```bash
curl --request POST "${API_ADDRESS}/group/transfer_group" \
...
| oldOwnerUserID | Yes | string | Current group owner. |
| newOwnerUserID | Yes | string | New owner, who must already be a group member. |
