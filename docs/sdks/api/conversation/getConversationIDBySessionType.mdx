...
```ts showLineNumbers
IMSDK.asyncApi('getConversationIDBySessionType', operationID: string, sourceID: string, sessionType: SessionType): Promise<string>
```
...
IMSDK.asyncApi('getConversationIDBySessionType', Date.now(), 'userID', 1)
  .then(({ data }) => {
    // 调用成功
  })
  .catch(({ errCode, errMsg }) => {
