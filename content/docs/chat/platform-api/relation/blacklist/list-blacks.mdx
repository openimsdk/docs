...
description: "List blacklisted users with the OpenIM Platform API, including endpoint, request body, response fields, and error handling."
...
version: "v3"
sourcePath: "/platform-api/relation/blacklist/list-blacks"
---

Use **List blacklisted users** to page through the blacklist owned by a specified user.

## HTTP request

```bash
POST {API_ADDRESS}/friend/get_black_list
```

### Request example

```bash
curl --request POST "${API_ADDRESS}/friend/get_black_list" \
...
| data.blacks | array | Blacklist records. Each item follows [`BlackInfo`](/platform-api/relation/overview#blackinfo). |
