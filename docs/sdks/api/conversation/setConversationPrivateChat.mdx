...
```ts showLineNumbers
IMSDK.asyncApi('setConversationPrivateChat', operationID: string, conversationID: string, isPrivate: boolean): Promise<void>
```
...
IMSDK.asyncApi('setConversationPrivateChat', Date.now(), 'conversationID', true)
  .then(({ data }) => {
    // 调用成功
  })
  .catch(({ errCode, errMsg }) => {
