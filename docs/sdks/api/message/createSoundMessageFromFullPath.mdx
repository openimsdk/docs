...
try {
  const data = await OpenIMSDKRN.createSoundMessageFromFullPath(
    {
      soundPath: 'soundPath_value',
      duration: 60, // Replace with the actual duration value
    },
    'OperationID_value'
  );
  console.log("createSoundMessageFromFullPath", data);
} catch (error) {
  console.error('Error createSoundMessageFromFullPath:', error);
}
