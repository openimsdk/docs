...
普通消息与只在线消息通过 `OIMAdvancedMsgListener` 接收。每次 callback 携带一个可空的 `OIMMessageInfo`，没有独立的离线新消息 selector；重新登录后的离线变化由 SDK 同步。应用先解析消息所属会话，再按 `conversationID:clientMsgID` 幂等合并。
...
完整的调用条件、请求结果与事件数据见[修改消息](/sdk/ios/message/managing-messages/modify-a-message)与[置顶或取消置顶消息](/sdk/ios/message/managing-messages/set-message-pinned)。
...
消息修改、置顶会话消息和音频转写来自 enterprise SDK，并在对应页面标为商业版。iOS 高阶 API 不提供创建定向群消息的 `OIMMessageInfo` factory selector。
