...
- The callback direction is from OpenIMServer to the app's backend via HTTP/HTTPS POST requests.
...
- The callback occurs before OpenIMServer prepares to create a group chat.
- The callback occurs before OpenIMServer prepares to approve a user joining the group.
