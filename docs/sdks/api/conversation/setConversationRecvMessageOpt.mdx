...
| status             | int           | 是       | 接收消息状态 0：正常；1：不接受消息；2：接受在线消息不接受离线消息 |
...
| status          | [OIMReceiveMessageOpt](docs/sdks/enum/recvMsgOpt.mdx) | 是       | 接收消息状态 |
...
```ts showLineNumbers
IMSDK.asyncApi('setConversationRecvMessageOpt', operationID: string, conversationID: string, opt: MessageReceiveOptType): Promise<void>
```
...
IMSDK.asyncApi('setConversationRecvMessageOpt', Date.now(), 'conversationID', 2)
  .then(({ data }) => {
    // 调用成功
  })
  .catch(({ errCode, errMsg }) => {
