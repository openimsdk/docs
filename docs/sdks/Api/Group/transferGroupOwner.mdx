...
```ts showLineNumbers
IMSDK.asyncApi('transferGroupOwner', operationID: string, groupID: string, newOwnerUserID: string): Promise<void>
```
...
IMSDK.asyncApi('transferGroupOwner', Date.now(), 'groupID', 'userID')
  .then(() => {
    // 调用成功
  })
  .catch(({ errCode, errMsg }) => {
