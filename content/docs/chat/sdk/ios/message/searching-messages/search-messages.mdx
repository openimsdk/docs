...
| `keywordList` | `NSArray<NSString *> *` | No | The keyword array. The pinned version currently supports one keyword only. |
| `keywordListMatchType` | `NSInteger` | Value type | A multi-keyword matching field marked as currently unused in the pinned version. |
| `senderUserIDList` | `NSArray * _Nullable` | Yes | A sender-filter field marked as currently unused in the pinned version. |
