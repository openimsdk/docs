...
| ---------------- | -------- | -------- | ------------------------------------------------------------------------------------ |
| conversationID   | string   | Yes       | Conversation ID                                                                              |
| startClientMsgID | string   | Yes       | 起始的消息 clientMsgID，第一次拉取为""，后续为上一次拉取的最后一条消息的 clientMsgID |
| count            | number   | Yes       | 一次拉取的数量                                                                       |
| lastMinSeq       | number   | Yes       | lastMinSeq Yes上一次拉取回调给的值，上下文，第二次拉取需要回传                        |
...
| Parameter Name        | Parameter Type                                                                             | Description        |
| --------------- | ------------------------------------------------------------------------------------ | ------------ |
| Promise.then()  | Promise<[AdvancedGetMessageResult](docs/sdks/class/message/advancedHistoryInfo.mdx)> | Callback on successful call |
...
try {
  ...
  console.error('Error getAdvancedHistoryMessageList:', error);
}
