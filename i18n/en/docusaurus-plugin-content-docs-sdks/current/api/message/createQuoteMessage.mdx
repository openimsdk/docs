...
| message     | [MessageItem](docs/sdks/class/message/messageInfo.mdx) | Yes       | 被引用消息                                              |
...
try {
  const data = await OpenIMSDKRN.createQuoteMessage(
    {
      text: 'Quoted Text',
      message: {
        ...
  ...
  console.log("createQuoteMessage", data);
} catch (error) {
  console.error('Error createQuoteMessage:', error);
}
```
