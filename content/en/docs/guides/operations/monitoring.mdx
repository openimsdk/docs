...
OpenIMServer uses Prometheus, Alertmanager, Grafana, and node-exporter for monitoring and alerting. A regular `docker compose up -d` does **not** start them. Enable the monitoring profile with:
...
### Import OpenIMServer metrics into Grafana
...
Import the default OpenIMServer dashboard in either of these ways:
...
![Load the OpenIMServer dashboard](/assets/guides/gettingStarted/assets/dashboard2.png)

Select the data source you just added, then select **Import**.

![OpenIMServer metrics dashboard](/assets/guides/gettingStarted/assets/dashboard3.png)

The main OpenIMServer monitoring metrics are now configured.
