...
When the Promise resolves, `data` is `RtcInviteResults`. For field descriptions, see [Start a one-to-one call](/sdk/wasm/calling/managing-calls/start-single-call).
