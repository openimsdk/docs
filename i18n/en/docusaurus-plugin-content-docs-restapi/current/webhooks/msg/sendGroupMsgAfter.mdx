...
## Description
The App backend can use this callback to perform actions on Group Chat messages, such as:
- Logging Group Chat messages in real-time (for logs or synchronization with other systems).
- Performing data analysis on Group Chat messages (e.g., counting users or messages).
...
- A user sends a Group Chat message through the client.
- An administrator sends a Group Chat message via the REST API.

## Timing of Callback
- After OpenIMServer receives the user’s Group Chat message and distributes it to the target user.
...
| callbackCommand     | string   | Callback command, here used for the callback after sending a Group Chat message |
