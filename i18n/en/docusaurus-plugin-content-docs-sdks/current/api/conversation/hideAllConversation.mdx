...
```ts showLineNumbers
OpenIMSDKRN.hideAllConversations(
  operationID: string
);
...
try {
  ...
  await OpenIMSDKRN.hideAllConversations(operationID);

  console.log("hideAllConversations: All conversations hidden");
} catch (error) {
  console.error('Error hideAllConversations:', error);
}
