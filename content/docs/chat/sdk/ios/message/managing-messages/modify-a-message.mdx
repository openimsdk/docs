...
The success callback's `NSString *` is not the modified message object. Success means that the current modification request has completed. The caller can optimistically display the submitted complete message by `clientMsgID`, then reconcile it through a modification event or a new query.
