...
try {
  const conversationIDList = ['ID1', 'ID2', 'ID3']; // Replace with the actual conversation IDs
  const operationID = 'OperationID_value';

  const data = await OpenIMSDKRN.getMultipleConversation(
    conversationIDList,
    operationID
  );

  console.log("getMultipleConversation", data);
} catch (error) {
  console.error('Error getMultipleConversation:', error);
}
