...
2. Obtain the `userID`, token, `apiAddr`, and `wsAddr` from the application backend.
...
Obtain the current user's `userID` and token, together with the environment's `apiAddr` and `wsAddr`, from the application backend. See [Before you start](/sdk/flutter/getting-started/before-you-start) for the required data.

`userID` is an OpenIMSDK user identifier and must correspond to the token. The mobile client consumes credentials returned by the backend; it does not create OpenIM users or issue tokens.
...
`initSDK()` retains the `OnConnectListener`, and the SDK does not expose a method for removing it. Keep one listener in a central owner and avoid reinitializing the SDK during widget rebuilds.
...
| `token` | `String` | Yes | The current user's token returned by the application backend. |
...
When a token expires or becomes invalid, obtain new credentials from the application backend and log in again or enter the application's sign-in flow. `onKickedOffline` indicates that another device or the server forced the current account offline; it is distinct from an explicit user sign-out.
