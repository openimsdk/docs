...
```ts showLineNumbers
IMSDK.asyncApi('createQuoteMessage', operationID: string, text: string, message: MessageItem): Promise<MessageItem>
```
...
IMSDK.asyncApi('createQuoteMessage', Date.now(), 'text', message)
  .then(({data}) => {
    // 调用成功
  })
  .catch(({ errCode, errMsg }) => {
