...
```ts showLineNumbers
OpenIMSDKRN.deleteConversationAndDeleteAllMsg(
...
try {
  ...
  const data = await OpenIMSDKRN.deleteConversationAndDeleteAllMsg(operationID,"conversationID");

  console.log("deleteConversationAndDeleteAllMsg", data);
} catch (error) {
  console.error('Error deleteConversationAndDeleteAllMsg:', error);
}
```
