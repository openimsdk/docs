...
```ts showLineNumbers
IMSDK.asyncApi('setConversationBurnDuration', operationID: string, conversationID: string, burnDuration: number): Promise<void>
```
...
IMSDK.asyncApi('setConversationBurnDuration', Date.now(), 'conversationID', 60)
  .then(({ data }) => {
    // 调用成功
  })
  .catch(({ errCode, errMsg }) => {
