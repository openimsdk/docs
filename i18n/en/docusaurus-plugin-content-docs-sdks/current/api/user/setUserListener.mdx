...
| 参数名称 | 参数类型                                              | 描述                   |
| -------- | ----------------------------------------------------- | ---------------------- |
| listener | [OnUserListener](/listener/userListener.mdx) | 用户信息更新的回调函数 |
...
| onUserStatusChanged | [OIMUserStatusInfo](/class/user/userStatusInfo.mdx) | 用户状态更新的回调函数 |
...
登录成功后自动设置，可以直接在客户端使用监听，[用户事件详情参见](/listener/userListener.mdx)
...
初始化成功后自动设置 回调会通过`globalEvent`传递到客户端，[用户事件详情参见](/listener/userListener.mdx)
