...
```ts showLineNumbers
IMSDK.asyncApi('createFileMessage', operationID: string, filePath: string,
  fileName: string): Promise<MessageItem>
```
...
IMSDK.asyncApi('createFileMessage', Date.now(), 'filepath', 'fileName')
  .then(({ data }) => {
    // 调用成功
  })
  .catch(({ errCode, errMsg }) => {
