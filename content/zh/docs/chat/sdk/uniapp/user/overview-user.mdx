...
`OpenIMUserInfo` 的公共字段包括 `userID`、`nickname`、`faceURL`、`ex` 和可选 `createTime`。`attachedInfo` 与 `globalRecvMsgOpt` 是<span className="enterprise-field-badge">商业版字段</span>，读取前应判空，不要假定公共服务端一定返回。
