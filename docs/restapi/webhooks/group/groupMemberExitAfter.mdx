...
## 功能说明
App 业务服务端可以通过该回调实时查看用户的退群动态，包括：对用户退群进行实时记录（例如记录日志，或者同步到其他系统）。
...
| CallbackCommand     | 固定值为：callbackAfterGroupMemberExitCommand                |
...
```json
{
  "callbackCommand": "callbackAfterGroupMemberExitCommand",
  "operationID": "123456",
  "groupID": "G001",
  "userID": "user123",
  "groupType": 1,
...
| 对象                     | 类型   | 描述                                                    |
|-------------------------|--------|----------------------------------------------------------------|
| callbackCommand         | string | 回调命令，这里是群组成员退出后的回调                             |
| operationID             | string | 操作的唯一标识符                                               |
| groupID                 | string | 群组的唯一标识符                                               |
| userID                  | string | 退出群组的用户的唯一标识符                                       |
| groupType               | int32  | 群组的类型                                                     |
| exitType                | string | 退出类型，取值有："self"表示用户主动退出，"kick"表示被踢出群组      |
| muteEndTime             | int64  | 用户被禁言的截止时间戳（毫秒），如果未被禁言则为空            |
