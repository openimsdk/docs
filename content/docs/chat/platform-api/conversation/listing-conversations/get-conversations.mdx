...
description: "Get conversations with the OpenIM Platform API, including endpoint, request body, response fields, and error handling."
...
Use **Get conversations** to retrieve selected conversation records owned by one user.
...
| --- | --- | --- | --- |
| ownerUserID | Yes | string | User who owns the conversation records. |
| conversationIDs | Yes | array | List of OpenIM conversation IDs. |
| hash <span className="enterprise-field-badge">Enterprise</span> | No | uint64 | Hash of the caller's current conversation set. Omit it for a normal query. |
...
| data.conversations | array | Matching records. Each item follows [`ConversationInfo`](/platform-api/conversation/overview#conversationinfo). |
| data.hash <span className="enterprise-field-badge">Enterprise</span> | uint64 | Server hash that can be supplied to a later query. |
