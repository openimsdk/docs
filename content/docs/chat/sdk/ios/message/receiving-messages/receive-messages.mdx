...
Common `OIMMessageInfo` properties used for reception, routing, and merging are:
...
Register the global message listener after `initSDK` succeeds and before `login`. Remove the same instance when signing out, switching accounts, or destroying the state layer. The callback message can be null, so the example also validates `clientMsgID`. Each callback supplies one `OIMMessageInfo`, not an array, and there is no separate offline-new-message selector.
