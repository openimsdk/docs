...
These commonly used receive, routing, and merge fields are nullable except for `exMap`, so a listener needs to handle missing values:
...
Add these three functions to the application's single `OnAdvancedMsgListener`. See [Event overview](/sdk/flutter/events/overview-events) for centralized configuration. The SDK does not expose a remove or unset API.
...
- Confirm that payloads missing required routing fields or `clientMsgID` are ignored instead of causing a crash.
