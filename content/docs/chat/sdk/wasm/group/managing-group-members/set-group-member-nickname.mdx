...
When the Promise resolves, the server has completed the request. When `OnGroupMemberInfoChanged` arrives, use `groupID:userID` to update the matching member. For the complete listener implementation, see [List group members](/sdk/wasm/group/retrieving-group-members/get-group-member-list).
