...
A successful callback means that the setting request completed; it does not mean that every client has updated its UI. `onGroupInfoChanged` provides the updated `GroupInfo`. Merge the verification setting by `groupID`. See [Group overview](/sdk/android/group/overview-group) for the complete listener setup.
