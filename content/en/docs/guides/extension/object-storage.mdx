...
- When using an Nginx proxy for the API, add `proxy_set_header X-Request-Api $scheme://$host/api;`. See [Domain configuration](../deployment/domain).
...
2. For MinIO behind Nginx, see [Domain configuration](../deployment/domain).
```yaml
