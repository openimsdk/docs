...
| platform   | [IMPlatform](/docs/sdks/Enum/IMPlatform.mdx) | 是       | 当前平台 |
| apiUrl   | String | 是       | SDK api地址 |
| wsUrl   | String | 是       | SDK WebSocket地址 |
| storageDir   | String | 是       | SDK数据库存储目录 |
| logLevel   | int | 是       | 日志打印等级 1不打印 |
| listener   | [OnConnectListener](docs/sdks/Listener/ConnectListener.mdx) | 是       | 监听 |
| isExternalExtensions   | bool | 不是       | 日志输出到文件 |
| isLogStandardOutput   | bool | 不是       | 日志输出到控制台 |
| logFilePath   | String | 不是       | 日志输出目录 |

### 代码示例

```java showLineNumbers
     OpenIMClient.getInstance().initSDK(application, 2,
