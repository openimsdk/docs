...
```ts showLineNumbers
IMSDK.asyncApi('createSoundMessage', operationID: string, soundPath: string,
  duration: number): Promise<MessageItem>
```
...
IMSDK.asyncApi('createSoundMessage', Date.now(), 'soundPath', 6)
  .then(({ data }) => {
    // 调用成功
  })
  .catch(({ errCode, errMsg }) => {
