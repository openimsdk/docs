...
The OpenIM WASM SDK does not provide a separate method that disconnects only the WebSocket while retaining the signed-in session. Use `logout()` to deliberately end the current user's session. The SDK wrapper handles network recovery and page foreground/background transitions; continue to use connection events to update application state.
