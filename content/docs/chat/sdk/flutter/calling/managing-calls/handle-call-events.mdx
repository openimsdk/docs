...
`SignalingManager` retains only one listener, so a later call replaces the previous instance. The SDK does not expose a removal method. When switching accounts, stop writing events into the old account's state and replace the listener with the complete instance for the new account.
