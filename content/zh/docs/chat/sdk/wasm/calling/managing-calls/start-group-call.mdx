...
每次群通话都应生成新的 `roomID`，不要直接复用 `groupID`，否则同一群组的不同通话可能被错误合并。被邀请方从邀请事件取得该值，后续接受、拒绝、取消和挂断操作都继续使用同一个 `roomID`。发起前还应排除当前用户、空值和重复成员。

Promise 成功后，`data` 是 `SignalingInviteResult`，字段含义见[发起单聊通话](/sdk/wasm/calling/managing-calls/start-single-call)。
