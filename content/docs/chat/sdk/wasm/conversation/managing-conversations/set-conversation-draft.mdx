...
When `setConversationDraft()` succeeds, the draft has been saved. The SDK synchronizes the changed conversation through `SdkEvent.OnConversationChanged`. The event's `data` is `ConversationItem[]`; merge `draftText` and `draftTextTime` by `conversationID`.
