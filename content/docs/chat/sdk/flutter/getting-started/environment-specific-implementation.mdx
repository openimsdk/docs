...
| `logLevel` | `int` | No | SDK log level. The default is `6`. |
...
- HTTPS and WSS endpoints require valid certificates. App Transport Security settings affect iOS network access.
- Test push delivery and background restoration on physical devices because simulator behavior differs.

## Lifecycle and cleanup

The Flutter SDK does not expose Dart methods for reporting network status or foreground and background transitions. When the app resumes, rely on the connection listener and query the snapshots required by the current page again. When the application no longer uses the SDK, call:
