...
try {
  ...
  const data = await OpenIMSDKRN.getJoinedGroupList(operationID);

  console.log("getJoinedGroupList received data", data);
} catch (error) {
  console.error('Error getJoinedGroupList:', error);
}
